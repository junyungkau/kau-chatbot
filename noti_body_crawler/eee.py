...
# 임베딩할 크롤링 결과 폴더들
TEXT_FILES_PATHS = [
    os.path.join(CURRENT_DIR, "KAU_Crawling_Data"),
    os.path.join(CURRENT_DIR, "KAU_College_Crawling_Data"),
]

DB_FAISS_PATH = os.path.join(CURRENT_DIR, "faiss_index")
DB_BM25_PATH = os.path.join(CURRENT_DIR, "bm25_retriever.pkl")

# 파일 내용 해시 저장용
# 구조:
# {
#   "KAU_Crawling_Data/xxx.txt": "sha256...",
#   "KAU_College_Crawling_Data/xxx.txt": "sha256..."
# }
INDEXED_FILES_LOG = os.path.join(CURRENT_DIR, "indexed_files.pkl")

# FAISS에 들어간 파일별 chunk id 저장용
# 구조:
# {
#   "KAU_Crawling_Data/xxx.txt": {
#       "file_hash": "sha256...",
#       "chunk_ids": ["KAU_Crawling_Data/xxx.txt::chunk::0", ...],
#       "chunk_count": 3,
#       "updated_at": "..."
#   }
# }
EMBEDDING_MANIFEST_LOG = os.path.join(CURRENT_DIR, "embedding_manifest.pkl")
...
def get_all_txt_files():
    ...
    for text_files_path in TEXT_FILES_PATHS:
        if not os.path.exists(text_files_path):
            print(f"[알림] 폴더 없음, 건너뜀: {text_files_path}")
            continue

        found_files = glob.glob(
            os.path.join(text_files_path, "**", "*.txt"),
            recursive=True
        )
    ...
    # 중복 제거 + 정렬
    all_txt_files = sorted(list(dict.fromkeys(all_txt_files)))
...
def get_current_file_hashes(all_txt_files):
    ...
    for file_path in all_txt_files:
        try:
            ...
            current_hashes[normalized_path] = make_file_hash(file_path)
        except Exception as e:
...
def load_documents_from_files(file_list):
    ...
    for file_path in file_list:
        ...
        metadata = {
            "doc_id": normalize_file_path(abs_file_path),
            "title": title,
            "source_file": file_name,
            "file_path": normalize_file_path(abs_file_path),
...
def create_vector_db():
    abs_txt_files = get_all_txt_files()

    if not abs_txt_files:
        print("txt 파일이 없습니다. 작업을 종료합니다.")
        return

    # 내부 관리는 상대경로 기준으로 통일
    all_txt_files = [normalize_file_path(f) for f in abs_txt_files]

    indexed_hashes = load_pickle_dict(INDEXED_FILES_LOG)
    embedding_manifest = load_pickle_dict(EMBEDDING_MANIFEST_LOG)

    current_hashes = get_current_file_hashes(abs_txt_files)
